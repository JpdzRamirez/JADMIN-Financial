...
@section('sub_title', 'Editar Nota Contable: ' . $notaContable->prefijo . ' ' . $notaContable->numero)
	...
	<div class="card">
		<div class="card-body">
                <div class="row form-group">
                    ...
                    <div class="col-md-6">
                        <input type="text" name="tercero" value="{{$notaContable->tercero->documento}}" id="tercero" class="form-control terce" autocomplete="off">
                    </div>
                </div>
                <div class="row form-group">
                    <label for="fecha" class="col-md-2">Fecha</label>
                    <div class="col-md-6">
                        <input type="date" name="fecha" value={{$notaContable->fecha}} id="fecha" class="form-control">
                ...
                <div class="row form-group">
                    ...
                    <div class="col-md-6">
                        <textarea name="concepto" id="concepto" rows="3" class="form-control">{{$notaContable->concepto}}</textarea>
                    </div>	
                </div>
			<hr>
            <table class="table table-bordered">
                ...
                <tbody id="tb-movs">
                @for ($i=0; $i < count($notaContable->movimientos); $i++)
                    <tr id="{{$notaContable->movimientos[$i]->id}}">
                        <td><input type="text" id="{{$notaContable->movimientos[$i]->id}}-cuenta" value={{$notaContable->movimientos[$i]->cuenta->codigo}} class="form-control cta" autocomplete="off"></td>
                        <td id="{{$notaContable->movimientos[$i]->id}}-lbl">{{$notaContable->movimientos[$i]->cuenta->nombre}}</td>
                        <td><select id="{{$notaContable->movimientos[$i]->id}}-tipo" value={{$notaContable->movimientos[$i]->naturaleza}} class="form-control">
                                @if ($notaContable->movimientos[$i]->naturaleza == "Crédito")
                        ...
                        <td><input type="text" id="{{$notaContable->movimientos[$i]->id}}-valor" value={{$notaContable->movimientos[$i]->valor}} class="form-control"></td>
                        <td><input type="text" id="{{$notaContable->movimientos[$i]->id}}-tercero" value={{$notaContable->movimientos[$i]->tercero->documento}} class="form-control terce" autocomplete="off"></td>
                        @if ($i >= 2)
                            <td>
                                <button type="button" onclick="borrar({{$notaContable->movimientos[$i]->id}}, 0)" class="btn btn-sm btn-danger">Borrar</button>
                            </td>
                        @endif
                    </tr>
                @endfor
...
@section('script')
    <script>
        ...
        $(document).ready(function () {
            movimientos = {{$notaContable->movimientos->pluck("id")}};
        });

        $(document).on('keydown.autocomplete', '.terce', function () {
            $(this).autocomplete({
		...
		function agregar() {
            ...
            fila = fila + '<td><input type="text" id="nuevo-' + ids + '-tercero" value="{{$notaContable->tercero->documento}}" class="form-control terce" autocomplete="off"></td>';
			...
			if(editados.length > 0){
                ...
                $.ajax({
                    ...
                    if(data.respuesta == "success"){
                        ...
                        Swal.fire(
                            data.msj,
                            "La edición de la nota Contable falló",
                            data.respuesta
                        );
                    }
