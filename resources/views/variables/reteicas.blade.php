		...
		<div class="card-body">
            ...
            <div class="align-center">
				...
				<table class="table table-bordered" style="table-layout: fixed">
					...
					<tbody>
						@forelse($reteicas as $reteica)
							<tr>
								<td>{{ $reteica->id }}</td>
                                <td>{{ $reteica->concepto }}</td>                             
								<td>{{ number_format($reteica->porcentaje, 2) }}%</td>
								<td>
									<button onclick="editar({{$reteica->id}}, '{{$reteica->concepto}}', {{$reteica->porcentaje}}, {{$reteica->venta->codigo}}, {{$reteica->compra->codigo}})" class="btn btn-warning btn-sm">Editar</button>
								</td>
							</tr>
						@empty
...
@section('modal')
        ...
        <div class="modal-dialog" style="width: 40%">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Nuevo Reteica</h4>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <form action="/contabilidad/reteicas/nuevo" method="POST">
    ...
    <div id="ModEditar" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="Modal" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Editar Reteica</h4>
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <form action="/contabilidad/reteicas/editar" method="POST">
                    <input type="hidden" name="idreteica" id="idreteica">
                <div class="modal-body">
                    <div class="row form-group">
