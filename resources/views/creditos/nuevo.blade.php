            ...
            <form action="/creditos/solicitar" method="POST" id="formcredito" accept-charset="UTF-8">
                ...
                <div class="form-group row">
                    <div class="col-md-2 offset-md-2">
                        <button type="button" class="btn btn-sm btn-dark" onclick="agregarTipo();"><i class="fa fa-circle-plus"></i> Agregar</button>
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-4">
                        {{ Form::number('ejerciendo', null, ['required', 'class' => 'form-control']) }}
                    </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-group row">
                    ...
                    {{ Form::label('pareja', 'Tiempo con su pareja (años)', ['class' => 'col-md-2']) }}
                    <div class="col-md-4">
                        {{ Form::number('pareja', null, ['required', 'class' => 'form-control']) }}
                    </div>
...
@section('script')
    <script>
        ...
        function agregarTipo(){
            let rowTipo = '<div class="form-group row align-items-center"> <div class="col-md-3 col-sm-5 offset-md-2"><select name="tipo[]" class="form-control">';
            for (const key in tipos) {
                rowTipo = rowTipo + '<option value="' + tipos[key].id + '">' + tipos[key].nombre + '</option>';
            }
            rowTipo = rowTipo + ' </select></div><div class="col-md-3 col-sm-5"><input type="number" name="monto[]" class="form-control"></div>';      
            rowTipo = rowTipo + '<div class="col-md-1"><button type="button" class="btn btn-sm btn-danger"><i class="fa-solid fa-xmark"></i></button></div></div>';
            $("#divTipos").append(rowTipo);         
        }
        ...
        $("#formcredito").submit(function (e) { 
            if(validarform == 0){
                ...
                let montos = $("input[name='monto[]'");
                let total = 0;
                for (let index = 0; index < montos.length; index++) {
                    total = total +  Number(montos[index].value);            
                }
                $("#total").val(total);
