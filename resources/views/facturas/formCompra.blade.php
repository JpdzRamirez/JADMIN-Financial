...
@section('sub_title', 'Nueva Factura de Compra')

@section('sub_content')
	<div class="card">
		<div class="card-body">
			<div class="row form-group">
				<label for="tercero" class="col-md-2 label-required">Tipo de Factura</label>
				<div class="col-md-6">
					<select name="tipof" id="tipof" class="form-control">
						<option value="Compra">Factura de Compra</option>
						<option value="Ajustes">Factura de Compra Ajuste</option>
					</select>
				</div>
			</div>
			<div class="row form-group">
...
@section('script')
    <script>
				...
				$.ajax({
					...
					if(datos != null){
						...
						if(datos.reteiva != null){
							datos.reteiva.compra.valor = datos.iva.valor*(datos.reteiva.porcentaje/100);
		...
		$(document).ready(function() {
			$("#reteiva").closest('.form-group').hide();
		});

		$("#iva").change(function() {
			if ($(this).is(":checked")) {
				$("#reteiva").closest('.form-group').show();
			} else {
				$("#reteiva").closest('.form-group').hide();
			}
		});
		...
		function enviar() {
			if ($("#tercero").val() != "" && $("#concepto").val() != "" && movimientos.length > 0){
				...
				$.ajax({
					type: "post",
					url: "/contabilidad/facturas/compras/enviar",
