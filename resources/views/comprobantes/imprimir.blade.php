...
<html lang="es">
    ...
    <body>
        ...
        <table>
            ...
            <tr>
                <td style="background-color: lightgray; text-align: center">{{$comprobante->prefijo}} {{$comprobante->numero}}</td>
            </tr>
        </table>
        <hr>
        <div>
            <table style="border: 1px solid black" class="tb-tercero">     
                ...
                <tr>
                    <th>Tercero:</th>
                    <td>{{strtoupper($comprobante->tercero->nombre)}}</td>
                </tr>
                <tr>
                    <th>Concepto:</th>
            ...
            <table class="table tb-border" style="font-size: 8pt">
                ...
                @foreach ($comprobante->movimientos as $movimiento)
                    <tr>
                        ...
                        @if ($movimiento->tercero->tipo == 'Persona')
                            <td>CC {{$movimiento->tercero->documento}} - {{$movimiento->tercero->nombre}}</td>
                        @else
                            <td>NIT {{$movimiento->tercero->documento}} - {{$movimiento->tercero->nombre}}</td>
                        @endif
                ...
                <tr>
                    <td></td>
                    <td></td>
                    <th colspan="2">Sumas iguales</th>
                    <td>{{number_format($comprobante->valor, 2, ",", ".")}}</td>
                    <td>{{number_format($comprobante->valor, 2, ",", ".")}}</td>
                </tr>
            </table>
        </div>
